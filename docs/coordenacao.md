...
- **Em curso — 3e.1 (Export)**: `@rust-dev` despachado com spec completa da ADR — D2 transporte em 3 fases (coleta async → `zip` 2 sync em `spawn_blocking` → `ReaderStream`; `Stored` mídia/`Deflated` texto), D1 layout do pacote, D9 deps (`zip`/`tokio-util`/`serde_yaml` promovido; `StoragePort.get_to_file` em port+mock+s3), openapi 0.5.0→0.6.0 declarando SÓ export (contract≡router a cada commit), `PROTECTED_ROUTES` `[200,401,404,503]`. Branch `feat/datasets-export-import`.
