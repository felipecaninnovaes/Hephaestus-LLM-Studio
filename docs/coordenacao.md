...
- **Próximo passo na ordem**: F4.3 (em curso, `@rust-dev`: manager — fila central, recovery no boot, auto-adoção orchestrator-local, report/heartbeat, policy VRAM no-op, test-db estendido) → F4.4 → F4.5 ∥ F4.6 → F4.7 → F4.8 → F4.9.
