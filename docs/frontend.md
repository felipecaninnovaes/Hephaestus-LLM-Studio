...
- Jobs — IMPLEMENTADO (Fatia 4 e 5; spec 0.7.0 e 0.8.0, ADR-0007 e ADR-0008):
  ...
  - `POST /api/jobs/cleanup` (`lib/jobs.ts:cleanupJobs`): body `{olderThanDays?, statuses?}` (ambos opcionais) → 200 `{deleted,jobs,objectKeys}` | 400 `invalid_request` | 503 `queue_unavailable`. Tela `/jobs`: botão "Limpar antigos" abre modal com idade (7/30/90 dias ou todos os terminais, sem filtro de idade); sucesso → toast com `deleted` + refresh da lista.
