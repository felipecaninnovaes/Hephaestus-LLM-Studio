...
- **Próximo passo na ordem**: F4.2a (em curso, `@rust-dev`: rotas de LEITURA de jobs + telemetry + client do manager mockado + spec 0.7.0 dessas rotas) → F4.2b (escrita: POST /api/jobs/yolo + abort + tracing//ready) → F4.3 → F4.4 → F4.5 ∥ F4.6 → F4.7 → F4.8 → F4.9. Fontes: ADR-0007 D3/D4/D7/D9 (:233–284, :341–391, :409–423) e D12 (:746–751, split autorizado).
