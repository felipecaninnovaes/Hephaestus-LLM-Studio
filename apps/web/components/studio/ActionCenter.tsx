						...
						<div className="glass-card flex flex-col items-center gap-3.5 rounded-2xl p-8 text-center mt-6 border border-white/10">
							...
							<div className="max-w-sm space-y-1">
								...
								<p className="text-xs text-zinc-400 leading-relaxed">
									O Centro de Atividades reúne as notificações do sistema em
									tempo real — treinos e tarefas de processamento, falhas de
									execução e alertas de recursos do nó (VRAM e CPU).
								</p>
							</div>
											...
											>
												...
												{isExpanded && (
													<div className="mt-3 border-t border-white/10 pt-3 text-2xs font-mono space-y-3 bg-black/55 p-3.5">
														...
														{caps.samplesGallery &&
															jobExtraArtifacts &&
															jobExtraArtifacts.length > 0 && (
																<JobSamplesGallery
																	jobId={job.id}
																	artifacts={jobExtraArtifacts}
																	onDownload={(jId, art) =>
																		handleDownload(jId, art)
																	}
																/>
															)}
			...
			<ConfirmDialog
				...
				body={
					<p className="text-xs text-zinc-300">
						...
						({abortTarget?.id.slice(0, 8)}…)? O processo de inferência/treino
