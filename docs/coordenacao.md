...
- **Próximo passo na ordem**: F4.6 (em curso, `@infra-dev`: compose build trainer-yolo + envs S3_ORCH + heph-orchestrator no seaweedfs-s3.json + compose.integ docker) → F4.7 (frontend) → F4.8 (reviewer) → F4.9 (docs-sync) + verificação final E2E.
