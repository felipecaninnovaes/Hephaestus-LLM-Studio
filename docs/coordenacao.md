...
## Estado atual — 2026-09-07 (sessão 13: fatia 4 EM EXECUÇÃO — aceite P1–P4 dado; F4.0 spike despachado ao @infra-dev)
...
- **F4.0 EM CURSO** — spike despachado ao `@infra-dev` em branch descartável `spike/jobs-f4-0` (criada a partir de main): C1 escopo S3 por prefixo (incógnita = `List` em sub-prefixo, fallbacks da D2/R1), C2 trixie-slim × aws-lc-sys, C3 docker-run do trainer via socket do host (fallback EXEC_MODE=subprocess). Matriz esperada em `spike/JOBS-SPIKE.md` no ramo do spike; após o veredito: apêndice "Resultados do spike F4.0" na ADR-0007 commitado na `feat/jobs-v1` (harness fica descartável, padrão 3b.0/3f.0). graft rebuildado em background nesta sessão (modelo local do usuário estava desligado — causa do STALE/404).
...
- **Próximo passo na ordem**: F4.0 spike (em curso) → apêndice na ADR → commits F4.1–F4.9 conforme §D12 da ADR-0007.
