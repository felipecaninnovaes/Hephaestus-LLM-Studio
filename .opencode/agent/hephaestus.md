...
4. **Implemente via subagentes**, um por área, passando contexto COMPLETO no prompt (arquivos-alvo, trecho do contrato, convenções, critério de pronto). Eles rodam em modelo barato: seu prompt deve ser específico o bastante para não exigir inteligência extra — inclua o resultado do graft no prompt para eles não caçarem contexto.
