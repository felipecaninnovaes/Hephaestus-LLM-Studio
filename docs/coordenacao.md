...
- **Em curso — 3e.3 (UI)**: `@frontend-dev` despachado — Exportar habilitado (DatasetMenu + galeria, download real), modal Importar (file picker `.zip`, campo `title`, **409 → diálogo de irreversibilidade → re-envio `replace=true`**, resultado com contagens + "Abrir dataset importado"), REMOVE botão Importar do header de `/datasets` (P2), `lib/backup.ts` novo. Smoke visual no Chrome contra o principal reconstruído com a branch.
