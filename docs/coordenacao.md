...
## Estado atual — 2026-09-06 (sessão 6: main VERDE pós-merges 3d+fix; 3g iniciada — feat/datasets-management)

- **3g "gestão de amostras e classes" DESENHADA (`@architect`, ADR-0005 v2, NÃO implementada)**: disparada pelos 2 gaps abaixo, redesenhada após usuário pedir RESTAURAÇÃO na exclusão de imagem ("quero que tenha como restaurar"). Design final: (1) **classes** = `PUT /api/datasets/:id/classes` reconciliação **por id** (id presente = rename preservando classId das caixas; DELETE+INSERT seria FATAL — `boxes.class_id` é FK CASCADE e apagaria todas as caixas; achado crítico do architect), **409 `classes_in_use`** bloqueia remover classe em uso, idx renumerado pela ordem do array, cor derivada da paleta, dance de UNIQUE (name/idx) em transação; (2) **exclusão de imagem RESTAURÁVEL** = migration `0005_image_soft_delete.sql` (`images.deleted_at`, unique parcial `WHERE deleted_at IS NULL` substituindo `UNIQUE(dataset_id,filename)`, `heph_refresh_dataset_counters` filtrando deleted — trigger AFTER UPDATE recalcula de graça) + `DELETE /:id/images/:imageId` (soft, 204) + `POST /:id/images/:imageId/restore` (204; conflito de filename → rename `{stem}_restaurado{ext}` + `copy_object` nova na StoragePort → 200 `{filename}`; copy falha → 503) + `DELETE /:id/trash` (purge real, sweep best-effort por prefixo de imagem) + `GET /:id/images?deleted=true` (lixeira) + `Dataset.trashCount` derivado (badge); inventário fechado das ~10 queries que ganham `deleted_at IS NULL` (D13 do ADR). Spec **0.4.0** (3g antes da 3f; ADR-0004 recalibra 3f → 0.5.0, nota de texto no commit 3g.3). Cross-tab aceito pelo usuário (PUT boxes 400 → toast + reload, caixas cinza). Decisões do coordenador sobre as 2 abertas do architect: **copy_object na StoragePort** (D5 do bucket exige chaves legíveis; `mc mirror` da 3e espelha nomes corretos) e **toast com "Desfazer"** (estende Toast.tsx ~20 linhas com `action`, dismiss 6s; a garantia vem da lixeira, o gesto vem do toast). Plano: **3g.0** migration 0005 → **3g.1** PUT classes → **3g.2** lixeira backend → **3g.3** contrato/openapi 0.4.0 + filtros → **3g.4** ClassesModal → **3g.5** lixeira UI → **3g.6** docs-sync. Fora de escopo v1: GC automático da lixeira (R7), renomear dataset, re-upload, split manual, cor custom. **ORDEM DE MERGE FEITA (2026-09-06): main verde** (`9de7378` merge 3d + `2303a39` merge fix, pushados; CI verde na main) → eu abri `feat/datasets-management` de main e despacho 3g.0. Correção minha sobre o desenho do architect para o 3g.0: a migration SOZINHA quebra o upload existente (`ON CONFLICT (dataset_id, filename)` sem WHERE não casa com índice parcial → erro runtime + testes de integração de upload falham) — 3g.0 = migration + os 2 ajustes do handler de upload (`ON CONFLICT … WHERE deleted_at IS NULL` em `handlers.rs:647-649` + `AND deleted_at IS NULL` no dedupe lookup `:676`). Branch da 3g: `feat/datasets-management`.
- **Fix `fix/web-bbox-insecure` (`b390dda`, nasceu DE `feat/datasets-gallery` — o editor só existe na 3d não mergeada)**: usuário reportou "desenha bbox mas não cria ao soltar o mouse". **Causa 1 PROVADA em aba real** (Chrome dele, `http://10.15.10.3:3000`): acesso por IP LAN HTTP = non-secure context → `crypto.randomUUID` é `undefined` → `TypeError` no `onUp` DEPOIS de `setDraft(null)` (caixa some, sem erro visível). Nos smokes dos agentes (localhost:3000 = secure) funcionava. Fix: **`apps/web/lib/id.ts::newId()`** (randomUUID se existe, senão UUID v4 via `getRandomValues` — disponível em qualquer contexto). **Causa 2**: dataset sem classes descartava o desenho em silêncio (`if (!classId) return`) — bloqueado na origem em `onFrameMouseDown` com toast "Este dataset não tem classes — crie uma antes de desenhar caixas.". Review `@reviewer`: **APROVA COM NITS** (nenhum crítico/maior; menor 1 = sem UI de classes pós-criação, virou gap de produto; nits 2–3 opcionais: guard `crypto` morta no fallback, tool bbox não disabled como o Salvar). Provas pós-fix por IP: caixa criada com coords exatas (0.166667/0.133333/0.30/0.2333), autosave PUT 200, zero erros; sem classe: toast + 0 caixas. `npm run build --workspace=web` verde. `next-env.d.ts` regenerado pelo build foi restaurado (ruído do gerador — usuário o commitou em `560d3e7`). Aguardando merge (ordem: junto com/after `feat/datasets-gallery`).
