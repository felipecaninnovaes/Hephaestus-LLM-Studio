...
- Jobs — IMPLEMENTADO (Fatia 4 e 5; spec 0.7.0 e 0.8.0, ADR-0007 e ADR-0008):
  ...
  - Submit assíncrono — IMPLEMENTADO (ADR-0025, spec 0.29.0): `JobStatus`
    (`types/studio.ts`) ganha `preparing`|`dispatched`; labels "Preparando"/
    "Despachando" (`JobCard.tsx`, `/jobs/page.tsx`, `Badge.tsx`); `phase`
    `packaging_dataset` = "Empacotando dataset" (`JobProgressLive.tsx`);
    `friendlyJobError` (`types/studio.ts`) parseia `prepare_failed:<code>:<msg>`
    e expõe só o motivo; modais de submit (`TrainYoloModal`, `AutoLabelModal`,
    `AutoTrackerModal`, `ForjaDifusaoSetup`, `GenerationPanel`) tratam
    `result.status === "preparing"` (navega `/jobs?job=<id>`); `ActionCenter`,
    `JobCard` e `JobLogViewer` tratam `preparing`/`dispatched` como ativos
    (polling vivo, abort permitido); libs (`jobs.ts`, `autolabel.ts`,
    `autotracker.ts`) documentam 202 `{jobId,status: preparing|queued}`.
