...
- **UUID do job com substring "400" apaga silenciosamente report da outbox** → checagem de erro irrecuperável por substring no texto cru do erro capturava dígitos do UUID da URL (`/jobs/...400.../report`) em falhas transitórias de conexão → validar estritamente prefixo de status HTTP (`report status: 4xx`), nunca substring genérica. (fatia orchestrator-autonomia)
- **Abort em preparing é sobrescrito e job roda na GPU** → ao transicionar de preparing para running, recriar a struct `ActiveJobState` resetava o flag `cancelled` para false → atualizar `container_name` in-place ou abortar imediatamente se `is_cancelled()` for verdadeiro antes de disparar o container. (fatia orchestrator-autonomia)
...
- **Text encoder não desaloca da VRAM mesmo após del text_pipeline / del text_enc** → `pipe_kwargs["text_encoder"] = text_enc` e referências internas em `text_pipeline.components` mantêm contagem de referências > 0 no escopo local da função → SEMPRE executar `pipe_kwargs.clear()`, anular `text_pipeline.text_encoder = None`, iterar limpando `components` e invocar `release_memory()` para derrubar a VRAM para 0.01 GB. (fatia qwen-image-2.1)
- **LoRA residual vaza entre requisições em pipelines cacheados** → reutilizar pipeline em cache sem purga acumula adaptadores na VRAM e polui inferências subsequentes sem LoRA → SEMPRE invocar `unload_lora_weights` incondicionalmente no pipeline em cache antes de avaliar e carregar novos adaptadores (`if loras_effective:`). (fatia qwen-image-memory-leaks)
