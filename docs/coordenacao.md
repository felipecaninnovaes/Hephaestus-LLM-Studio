...
- **Próximo passo na ordem**: F4.4 (em curso, `@rust-dev`: orchestrator — cliente S3 escopado, dispatch, build, exec docker, artefatos) **∥ F4.5 (em curso, `@python-engines`: trainer-yolo mock treino)** — file ownership disjunto → F4.6 (infra) → F4.7 (frontend) → F4.8 → F4.9.
