...
## Resultados do spike 3f.0 (2026-09-07 — matriz completa em `spike/SEARCH-SPIKE.md`, commit `e07ce2d`)

**5/5 critérios PASSAM — D1/D2/D4 ficam aprovadas, com 3 correções que valem para os commits 3f.1–3f.5:**

1. **Tag da imagem: `pgvector/pgvector:pg16-trixie@sha256:c8483555ce48101872f888c1df8a895ff689d6c7c7a5f7ac266475f9dfe89e0b`** (pgvector 0.8.6, PG 16.15-1.**pgdg13**+2 — mesma build do `postgres:16` pinado). A tag `pg16` deste repo é **bookworm/glibc 2.36**: o PGDATA do `postgres:16` atual (trixie/glibc 2.41) abre, mas com collation version mismatch que quebra `CREATE DATABASE` (template1) e suspeita índices btree de texto. D2 acima: onde lê "`pgvector/pgvector:pg16`", leia **"`pgvector/pgvector:pg16-trixie`"** (digest acima). Regra: bases glibc de postgres↔pgvector nunca divergem.
2. **Crate: `pgvector = { version = "=0.4.1", features = ["sqlx"] }`** — o 0.4.2 (mai/2026) exige sqlx-core **0.9** e quebra o workspace (sqlx 0.8.6): dois sqlx no grafo, trait `Type` incompatível (14 erros). Com 0.4.1: round-trip 512d exato (`try_get::<Vector>`), `ORDER BY v <=> $1` provado. D10 (veto `query!`) segue válido — crate é bind runtime.
3. **Números provados**: HNSW 10k×512d — build **1.76 s** (limite 30), busca k=10 **p50 0.317 ms / p95 1.5 ms** (limite 50), **top-1 20/20** vs brute force (recall ≥ 90% garantido com folga); bulk insert 10k = 0.28 s (QueryBuilder, 1000 binds/stmt). Embedder mock (stdlib, sem torch): `/embed` batch 32 = **17.9 ms**, `/embed-text` = 2.6 ms (limite 500). `pg_dump -Fc → pg_restore` preserva tipo e **todos os valores** (`md5(v::text)` idêntico). Upgrade do volume real: `cp -a` do pgdata abre com recovery de WAL ~0.001 s (FATALs transitórios "starting up" são benignos) e `CREATE EXTENSION` roda no banco real migrado.
4. **Achados secundários**: com 10k linhas o planner prefere seq scan a HNSW (`LIMIT k` pequeno) — não forçar `enable_seqscan=off` em produção (foi recurso do benchmark); subscripting `v[i]` não existe no pgvector 0.8.6 (usar `v::text` em testes); `EXPLAIN (FORMAT JSON)` devolve coluna JSON (usar FORMAT TEXT). **Contrato do mock fixado e provado idêntico Rust×Python** (maxdiff 1.4e-17): `sha256-chain` → 512 f64 em [-1,1) → L2; `/embed-text` ecoa `id = índice`; detalhes no fim de `spike/SEARCH-SPIKE.md`.
