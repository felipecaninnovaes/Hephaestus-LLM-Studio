...
- **Roteamento por capacidade no manager (GPU/VRAM/kind) — ABERTA 2026-09-09 (ADR-0010 D2):** substituir o `LIMIT 1` arbitrário do `dispatch_next` por seleção por GPU/VRAM/kind (policy VRAM aplicada + `vram_min_gb` vs `orchestrators.vram_total_gb`). A sessão GPU v1 usa contrato operacional manual (stop local + DELETE row + INSERT remoto + `AUTO_ADOPT_LOCAL=0`); a dívida é o substituto correto.
...
- **`orchestrators.gpus`/`vram_total_gb` preenchidos por INSERT manual (ADR-0010 D1) — NOTA 2026-09-09:** a sessão GPU (TrueNAS) preenche essas colunas por INSERT manual na tabela (dado estático real do host). O manager NÃO as escreve (regra ADR-0009 D1 permanece). Necessário para a sessão GPU; quando a telemetria por orquestrador (R1 abaixo) chegar, o manager poderá preencher via heartbeat com identidade.
