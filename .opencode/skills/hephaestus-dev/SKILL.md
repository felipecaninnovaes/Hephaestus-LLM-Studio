...
- `docs/coordenacao.md` — estado do plano e próximo passo (ler no início de sessão).
- `docs/adr/` — ADRs aceitas são a especificação da fatia (ex.: 0003 = 3b inteira).
...
- **Coordenador (hephaestus) stageia e commiteia**, sempre fora de `main` (exceções autorizadas: `docs/coordenacao.md` e ADRs).
...
- **Um dispatch = um commit** (no máximo um passo numerado do plano da ADR, ex.: 3b.1). Nunca pedir "implemente a fatia inteira": modelo barato cumpre literalmente e produz monocommit.
...
- **Todo por passo**: cada todowrite espelha um dispatch/commit, com `in_progress` único e atualização imediata após a verificação do commit — nunca batch de conclusão, nunca todo-pai "implementar fatia X" vivo por horas (foi assim que a lista apodreceu na 3a).
- **Spikes ficam com o coordenador**: spike é pesquisa que produz especificação (charter do implementador é executar spec completa, não decidir). Rodar iteração de build/prova em **script bash único** que executa o ciclo e imprime a matriz de resultados — não gastar turno do modelo caro em loop de compilação (lição do spike 3b.0).
- Erro de build/teste → `@fixer` (máx. 2 tentativas); reincidiu, escalar a `@architect` ou assumir.
...
- Boundaries: principal é a única superfície do front; manager possui fila/VRAM; orchestrators são stateless. Postgres: principal dono de datasets/auth/settings; manager de jobs/runners/orchestrators. Estado reconstruído do banco no boot, nunca só memória.
